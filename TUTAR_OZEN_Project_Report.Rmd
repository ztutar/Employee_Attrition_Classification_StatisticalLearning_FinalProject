...
The above model statistics indicate that p-values of Company.Tenure and
Employee Recognition are above 0.5 indicating that these features are
insignificant to the results. Additionally, some Job.Roles and
Monthly.Income also have high p-values indicating that their effect on
...
```{r step_back}
# Backward Stepwise Search with BIC as penalty term
glm.bwd.AIC <- step(glm.FULL, direction="backward", k=2, trace=0, steps=20)
summary(glm.bwd.AIC)

R2.bwd.AIC <- 1 - (summary(glm.bwd.AIC)$deviance/summary(glm.bwd.AIC)$null.deviance)
R2.bwd.AIC

vif.bwd.AIC <- data.frame(features = names(vif(glm.bwd.AIC)), VIF = vif(glm.bwd.AIC),row.names = NULL)
vif.bwd.AIC[order(-vif.bwd.AIC$VIF), ]

```

It looks like backward stepwise search dropped "Job.RoleFinance",
"Job.RoleHealthcare", "Job.RoleTechnology", "Company.Tenure" and
"Employee.Recognition" from the model. These features were also the ones
with high p-values so it seems logical. But oddly the model kept the
"Job.RoleMedia" feature so this tells us that having a job role in Media
is significant to employee Attrition for this case.
...
decreased. Therefore the backward search did not converge to an optimal
model.
...
```{r NET}
# Elastic Net Regression with alpha=0.1 (more weight on Ridge Reg.)
glm.ELNET <- cv.glmnet(as.matrix(X.train), y.train, alpha = 0.1, 
