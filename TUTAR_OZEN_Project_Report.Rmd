...
data_corr <- data_detailed[, !names(data_detailed) %in% exclude_columns]
data_corr <- data_corr[, c(setdiff(names(data_corr), "Attrition"))]
...
for (page in 1:num_pages) {
  ...
  for (i in 1:plots_per_page) {
    ...
    boxplot(data[[num_var]] ~ data[[cat_var]], main=paste(num_var, "\nby", cat_var), xlab=cat_var, ylab=num_var, col=c("firebrick4","rosybrown2"))
...
for (var in numeric_var_names) {
  ...
  cat("\nOutlier values in", var, "are between:\n")
...
X <- data_normalized[, !(colnames(data_normalized) %in% c("Employee.ID", "AttritionStayed"))]

y <- data_normalized$AttritionStayed
```
...
```{r}
kable(prop.table(table(y.train)))
```
...
approximately `r R2*100`% of the variance in the target can be explained by
the features in the model. Since `r R2*100`% is relatively low, it suggests
...
Marital.StatusSingle and Marital.StatusMarried. These are mostly dummy features of nominal variables
...
vif.BACKWARD[order(-vif.BACKWARD$VIF), ]
```
...
With backward feature selection we were able to decrease the VIF scores, p-values and AIC score.
Although $R^2$ slightly decreased to `r 1 - (summary(glm.BACKWARD)$deviance/summary(glm.BACKWARD)$null.deviance)`, this is expectable since the number of features in the model decreased. But most importantly we decreased the variance inflation factor of the model so the model is more stable.
...
Although the AUC values are same, to better analyse the difference between the model performances we can
calculate and compare the evaluation metrics.
...
the test set. These results indicate that the model with Elastic Net regularization is better in generalization. Therefore we can select the glm.ELNET.predict as the best prediction for logistic regression model.
