...
The dataset is already splitted into train and test but
in order to better understand the data, it is crucial
to analyse the dataset as a whole.
...
representations by expanding factors to a set of dummy
variables. In order to avoid multicollinearity, one
dummy variable will be dropped.
...
data_dummy <- as.data.frame(data_dummy[,-1])  # -1 to remove the intercept column
...
As expected for each categorical variable a new dummy
column is created and one column for each category is
dropped to avoid "dummy variable trap".
...
X <- data_dummy[, !(colnames(data_dummy) %in% c("Employee.ID", "AttritionStayed"))]
...
labeled dataset where the input features (independent
